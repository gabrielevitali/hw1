<?php
    ...
    if (!empty($_POST["departureAirport"]) && !empty($_POST["arrivalAirport"]) && !empty($_POST["date"])){
    ...
    $arrivalAirport = mysqli_real_escape_string($conn, $_GET["arrivalAirport"]);
    $date = mysqli_real_escape_string($conn, $_GET["date"]);
    $query = "SELECT flightID, aeroportoPartenza, aeroportoArrivo, dataPartenza, oraPartenza, dataArrivo, oraArrivo, stato  FROM voli WHERE aeroportoPartenza = '$departureAirport' AND aeroportoArrivo = '$arrivalAirport' AND dataPartenza = '$date'";
